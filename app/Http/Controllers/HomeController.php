...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $monthlyreport = array();
        for ($i = 0; $i <= 11; $i++) {
            $month = Carbon::today()->startOfMonth()->subMonth($i+3);
            $year = Carbon::today()->startOfMonth()->subMonth($i+3)->format('Y');

                $price = Report::where('user_id',auth()->user()->id)
                ...
                array_push($monthlyreport, array(
        ...
        $totalearnings = Report::where('user_id',auth()->user()->id)
                        ->sum('money');
